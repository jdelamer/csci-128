...
\course{CSCI 128: Conding for Problem Solving}
...
\begin{frame}[fragile]{Introduction to Loops}
    \textbf{To work with many pixels, we need loops!}

    \vspace{1em}

    \textbf{The \texttt{for} loop with \texttt{range()}:}
...
# Output: 10, 11, 12, 13, 14
    ...
    \begin{itemize}
        \item \texttt{range(n)} generates numbers from 0 to n-1
        \item \texttt{range(start, stop)} generates numbers from start to stop-1
        \item Perfect for iterating over pixel coordinates!
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{Drawing a Line with a Loop}
    \textbf{Let's draw a simple horizontal line:}
...
# Draw a red horizontal line across the middle (y=50)
for x in range(100):
...
img.show()
    ...
    \begin{itemize}
        \item Loop runs 100 times: x = 0, 1, 2, ..., 99
        \item Each iteration sets one pixel to red at position (x, 50)
        \item Result: A horizontal red line across row 50
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{Drawing a Cross Pattern}
    \textbf{Now let's combine two loops to draw a cross:}
...
img.show()
    ...
    This creates a red cross pattern with a blue vertical line!
\end{frame}
