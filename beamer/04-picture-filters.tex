...
\begin{frame}{What is a Condition?}
    \begin{itemize}
        \item A \textbf{condition} (or \textbf{boolean expression}) is a question the computer can answer with \texttt{True} or \texttt{False}.
        \item Example questions:
        \begin{itemize}
            \item Is this number greater than 10?
            \item Is this pixel brighter than 128?
            \item Is this color equal to pure red?
        \end{itemize}
        \item In Python, conditions use \textbf{comparison operators}:
        \begin{itemize}
            \item \texttt{==} (equal), \texttt{!=} (not equal)
            \item \texttt{<}, \texttt{>}, \texttt{<=}, \texttt{>=}
        \end{itemize}
        \item Example: \texttt{x > 5} evaluates to \texttt{True} if \texttt{x} is 7, or \texttt{False} if \texttt{x} is 3.
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{\texttt{if}: Making Decisions}
    \begin{itemize}
        \item We use \texttt{if} to run code \textbf{only when} a condition is \texttt{True}.
        \item Syntax: \texttt{if condition:} (note the \textbf{colon}!)
    \end{itemize}
...
print("Done")
    ...
    \begin{itemize}
        \item The line under \texttt{if} is \textbf{indented} (4 spaces or 1 tab).
        \item It runs only if \texttt{x > 5} is \texttt{True}.
        \item \texttt{"Done"} prints every time (not indented = not part of \texttt{if}).
        \item \textbf{Output:} \texttt{x is big!} then \texttt{Done}
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{\texttt{if} and \texttt{else}}
    \begin{itemize}
        \item \texttt{else} tells Python what to do when the condition is \texttt{False}.
    \end{itemize}
...
print("Check complete")
    ...
    \begin{itemize}
        \item Exactly \textbf{one} of the two blocks runs:
        \begin{itemize}
            \item the \texttt{if} block (when \texttt{True}), or
            \item the \texttt{else} block (when \texttt{False}).
        \end{itemize}
        \item \textbf{Output:} \texttt{Above freezing} then \texttt{Check complete}
        \item If \texttt{temperature = -5}: \texttt{At or below freezing} then \texttt{Check complete}
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{Connecting to Images}
    \begin{itemize}
        \item We can use \texttt{if} and \texttt{else} to decide:
        \begin{itemize}
            \item Should this pixel be darker or lighter?
            \item Should this pixel become white or black?
            \item Is this pixel part of the background or the foreground?
        \end{itemize}
        \item Example idea:
    \end{itemize}
...
if pixel_brightness > 128:
    ...
    \begin{itemize}
        \item Next: we will write real code like this to create a threshold filter.
    \end{itemize}
\end{frame}
